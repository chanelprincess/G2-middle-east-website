...
import * as ProjectsAuth from './lib/auth'
import { requireProjectsAuth, logActivity } from './lib/projects-middleware'
...
app.post('/api/projects/auth/login', async (c) => {
  try {
    ...
    const token = ProjectsAuth.generateJWT(user.id as string, user.email as string, jwtSecret)
    
    // Create session
    const sessionId = nanoid()
    const sessionExpiry = remember_me ? 30 : 7 // 30 days if remember me, else 7 days
    const expiryDate = new Date()
    expiryDate.setDate(expiryDate.getDate() + sessionExpiry)
    
    await c.env.PROJECTS_DB.prepare(`
      INSERT INTO projects_sessions (id, user_id, token, expires_at, ip_address, user_agent)
      VALUES (?, ?, ?, ?, ?, ?)
    `).bind(
      sessionId,
      user.id,
    ...
    // Set cookie
    const cookieMaxAge = sessionExpiry * 24 * 60 * 60 // Convert days to seconds
    c.header('Set-Cookie', `projects_auth=${token}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=${cookieMaxAge}`)
